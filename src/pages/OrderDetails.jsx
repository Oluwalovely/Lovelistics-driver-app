...
const OrderDetails = () => {
    ...
    const sendCurrentLocation = () => {
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                try {
                    await sendLocation(orderId, { lat: position.coords.latitude, lng: position.coords.longitude, speed: position.coords.speed || 0 });
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            ...
            if (['picked-up', 'in-transit'].includes(next.value) && !tracking) startTracking();
                    ...
                    <div className="col-md-7">
                        ...
                        <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14 }}>
                            <div className="card-body">
                                ...
                                <p className="mb-0 small d-flex align-items-center gap-1">
                                    <Phone size={13} />
                                    <a href={`tel:${order.customer?.phone}`} style={{ color: '#e8610a' }}>{order.customer?.phone}</a>
                                </p>
                            </div>
                        </div>
                        ...
                        {['picked-up', 'in-transit'].includes(order.status) && (
                            <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14, borderLeft: `4px solid ${tracking ? '#198754' : '#e8610a'}` }}>
                                <div className="card-body d-flex justify-content-between align-items-center">
                                    <div>
                                        <h6 className="fw-bold mb-1 d-flex align-items-center gap-2" style={{ color: '#0d1f4f' }}><Radio size={16} /> Live Location Sharing</h6>
                                        <small className="text-muted">{tracking ? 'Your location is being shared with the customer' : 'Share your live location with the customer'}</small>
                                    </div>
                                    <button className="btn btn-sm fw-bold" onClick={tracking ? stopTracking : startTracking}
                        ...
                        {/* Status update */}
                        {next && (
                            <button className="btn w-100 fw-bold mb-2" onClick={handleUpdateStatus} disabled={updating}
                        ...
                        {order.status === 'delivered' && (
                            <div className="alert alert-success mt-2 text-center">Order delivered! Waiting for customer confirmation.</div>
                        )}
                        {order.status === 'confirmed' && (
                            <div className="alert mt-2 text-center" style={{ background: '#d1e7dd', color: '#0a3622' }}>Order confirmed by customer. Great job!</div>
                        )}

                        {['cancelled', 'confirmed'].includes(order.status) && (
                            <button className="btn btn-outline-danger w-100 d-flex align-items-center justify-content-center gap-2 mt-2"
